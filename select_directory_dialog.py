...
class SelectDirectoryDialog(QDialog):
    def __init__(self, parent, adb_handler, start_path="/storage/emulated/0"):
        super().__init__(parent)
        self.setWindowTitle("Select Destination Folder")
        self.resize(600, 400)
        self.adb_handler = adb_handler
        self.selected_path = None
        self.current_path = start_path.rstrip("/")
    ...
    def refresh(self):
        ...
        try:
            files = self.adb_handler.list_directory(self.current_path)
            if files is None:
